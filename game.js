...
const SNIPPETS = [
  ...
  // More HTML
  { code: '<input type="text" placeholder="Type here">', tip: 'HTML input field.' },
  { code: '<div class="container"></div>', tip: 'HTML div with class.' },
  { code: '<img src="knight.png" alt="Knight">', tip: 'HTML image tag.' },
  { code: '<a href="#">Start Quest</a>', tip: 'HTML link.' },
  // More CSS
  { code: '.glow { box-shadow: 0 0 8px #00ffe7; }', tip: 'CSS glowing effect.' },
  { code: 'font-family: "Press Start 2P", monospace;', tip: 'CSS font family.' },
  { code: 'background: linear-gradient(90deg, #23234a, #00ffe7);', tip: 'CSS gradient background.' },
  { code: '.enemy { transform: rotateY(20deg); }', tip: 'CSS 3D transform.' },
  // More JS
  { code: 'setTimeout(() => { attack(); }, 2000);', tip: 'JS timeout function.' },
  { code: 'const hp = Math.max(0, player.hp - 10);', tip: 'Math.max for health.' },
  { code: 'window.addEventListener("keydown", fn);', tip: 'Event listener.' },
  { code: 'let score = localStorage.getItem("score");', tip: 'Local storage.' },
  // Fantasy/fun
  { code: 'let sword = "Excalibur";', tip: 'Legendary sword variable.' },
  { code: 'if(monster.type === "dragon") { slay(); }', tip: 'Conditional for dragons.' },
  { code: '<p>Victory is yours, Knight!</p>', tip: 'Victory message.' },
  { code: '.magic { animation: sparkle 2s infinite; }', tip: 'CSS animation.' },
  { code: 'function castSpell(spell) { return spell(); }', tip: 'Function for magic.' },
  { code: 'let mana = 100;', tip: 'Mana variable.' },
  { code: 'const shield = { block: true };', tip: 'Shield object.' },
  { code: 'for(const enemy of enemies) { attack(enemy); }', tip: 'For-of loop.' },
  { code: 'let gold = 0;', tip: 'Gold variable.' },
  { code: 'if(player.hp <= 0) { gameOver(); }', tip: 'Game over check.' },
  { code: 'console.log("You are a Code Knight!");', tip: 'Console log.' },
];
